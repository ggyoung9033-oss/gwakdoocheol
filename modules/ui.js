    ...
    panel.innerHTML = `
        ...
        <div class="gwak-panel-body" data-pane="chat">
            ...
            <div class="gwak-input-area">
                <textarea class="gwak-input" id="gwak-input" placeholder="곽두철한테 뭐든 떠들어봐..." rows="2"></textarea>
        ...
        <div class="gwak-panel-body gwak-note-pane" data-pane="note" style="display:none;">
            ...
            <div style="font-size: 0.85em; opacity: 0.75; margin-bottom: 8px;">
                곽두철과 의논한 합의사항 박아두면 RP 채팅 LLM이 참고함.
            </div>
            ...
            <textarea id="gwak-note-content" rows="14" placeholder="예) 캐릭터 결 의논 내용..."></textarea>
...
function makeResizable(el) {
    ...
    function endResize() {
        ...
        const s = loadSettings(); s.panelWidth = el.offsetWidth; s.panelHeight = el.offsetHeight; saveSettings(s);
    }
    ...
    document.addEventListener('touchmove', (e) => { if (isResizing) { e.preventDefault(); moveResize(e.touches[0].clientX, e.touches[0].clientY); } }, { passive: false });
...
function toggleMinimize() {
    ...
    if (btn) { btn.textContent = isMin ? '▲' : '▼'; btn.title = isMin ? '펼치기' : '최소화'; }
}

function adjustOpacity(delta) {
    const s = loadSettings(); const cur = s.opacity ?? 100;
    ...
    s.opacity = newVal; saveSettings(s);
}
...
async function handleSend() {
    ...
    try {
        ...
        const { messages } = await buildMessages({
            userInput,
            gwakHistory: (record?.messages || []).slice(0, -1),
            persona: settings.systemPrompt,
            contextOptions: { recentChatN: settings.recentChatN },
        });
        const reply = await sendRequest(messages, { maxTokens: settings.maxTokens, profileId: settings.profileId });
        loadingEl.remove();
        renderMessage({ role: 'assistant', content: reply });
        await dbAppendMessage(getCurrentChatKey(), { role: 'assistant', content: reply }, {});
    } catch (err) {
        loadingEl.remove(); renderError(err?.message || String(err));
    } finally { isLoading = false; }
}

async function handleReset() {
    if (!confirm('곽두철과의 모든 대화를 진짜 리셋할까?')) return;
    await dbDeleteHistory(getCurrentChatKey());
    panel.querySelector('#gwak-thread').innerHTML = '';
    renderMessage({ role: 'assistant', content: '🐗 리셋 완료! 새로 시작하자.', isWelcome: true });
}
...
function toggleSettings() {
    ...
    if (settingsPane.style.display !== 'none') { showPane('chat'); return; }
    const s = loadSettings();
    ...
    refreshPanelProfileDropdown(); showPane('settings');
}

function toggleNote() {
    const notePane = panel.querySelector('[data-pane="note"]');
    if (notePane.style.display !== 'none') { showPane('chat'); return; }
    const n = loadNote();
...
function handleSaveNote() {
    ...
    if (window.toastr) window.toastr.success('🐗 노트 저장됨');
}

function handleClearNote() {
    if (!confirm('노트 비울까?')) return;
    clearNoteData(); panel.querySelector('#gwak-note-content').value = '';
...
function refreshPanelProfileDropdown() {
    ...
    const settings = loadSettings();
    select.innerHTML = '<option value="">기본 활성 프로필</option>';
    profiles.forEach(p => {
        const opt = document.createElement('option');
        opt.value = p.id; opt.textContent = p.name;
        if (p.id === settings.profileId) opt.selected = true;
...
function handleSaveSettings() {
    ...
    saveSettings({
        ...s,
        recentChatN: parseInt(panel.querySelector('#gwak-recent-n').value) || 10,
        maxTokens: parseInt(panel.querySelector('#gwak-max-tokens').value) || 4096,
        systemPrompt: panel.querySelector('#gwak-system-prompt').value,
    });
    if (window.toastr) window.toastr.success('🐗 설정 저장됨');
    showPane('chat');
}
...
function renderMessage({ role, content, isWelcome }) {
    ...
    el.innerHTML = `<div class="gwak-msg-avatar">${role === 'user' ? '🙂' : '🐗'}</div><div class="gwak-msg-content"></div>`;
...
function renderError(msg) {
    ...
    el.innerHTML = `<div class="gwak-msg-content">⚠️ ${msg}</div>`;
    thread.appendChild(el); thread.scrollTop = thread.scrollHeight;
}
...
async function loadHistory() {
    try {
        ...
        if (!record || !record.messages?.length) {
            renderMessage({ role: 'assistant', content: '🐗 야 처음 보네 ㅋㅋ. 뭐 도와줄까?', isWelcome: true });
        } else {
            record.messages.forEach(m => renderMessage(m));
        }
    } catch (e) { console.error('[곽두철] loadHistory 실패:', e); }
}
...
function makeDraggable(el) {
    ...
    header.addEventListener('mousedown', (e) => { if (!e.target.closest('button')) start(e.clientX, e.clientY); });
    document.addEventListener('mousemove', (e) => move(e.clientX, e.clientY));
    document.addEventListener('mouseup', () => isDragging = false);
    header.addEventListener('touchstart', (e) => { if (!e.target.closest('button')) start(e.touches[0].clientX, e.touches[0].clientY); }, { passive: true });
    document.addEventListener('touchmove', (e) => { if (isDragging) move(e.touches[0].clientX, e.touches[0].clientY); }, { passive: true });
...
export function showPanel() {
    createPanel();
    panel.removeAttribute('style'); // 누적된 스타일 리셋

    const s = loadSettings();
    const isMobile = window.innerWidth <= 768;

    if (isMobile) {
        // 모바일 강제 고정 좌표 및 사이즈 적용
        panel.style.cssText = "position:fixed !important; bottom:10px !important; left:50% !important; transform:translateX(-50%) !important; width:92vw !important; height:75dvh !important; display:flex !important; z-index:10001;";
    } else {
        const maxW = window.innerWidth - 40;
        const maxH = window.innerHeight - 40;
        panel.style.width = Math.min(s.panelWidth || 380, maxW) + 'px';
        panel.style.height = Math.min(s.panelHeight || 540, maxH) + 'px';
    ...
    if (panel.classList.contains('gwak-minimized')) {
        ...
        if (minBtn) { minBtn.textContent = '▼'; minBtn.title = '최소화'; }
    }
    showPane('chat');
    loadHistory(); // 현재 채팅방 데이터 로드 
    console.log('[곽두철] 패널 표시 (ID:', getCurrentChatKey(), ')');
}

export function hidePanel() { if (panel) panel.style.display = 'none'; }
export function togglePanel() { if (!panel || panel.style.display === 'none') showPanel(); else hidePanel(); }
        ...
        if (ctx.eventSource && ctx.event_types?.CHAT_CHANGED) {
            ctx.eventSource.on(ctx.event_types.CHAT_CHANGED, () => {
                if (panel && panel.style.display === 'flex') {
                    loadHistory(); // 채팅 전환 시 히스토리 즉시 교체 
                }
            });
        }
    } catch (e) { console.warn('[곽두철] CHAT_CHANGED listener 실패:', e); }
}

export function setupExtensionCard() {
    if (document.getElementById('gwak-ext-card')) return;
    const container = document.getElementById('extensions_settings');
    if (!container) { setTimeout(setupExtensionCard, 1000); return; }
    ...
    <div id="gwak-ext-card" class="inline-drawer">
        ...
        <div class="inline-drawer-content" style="display: none; padding: 10px;">
            <div style="margin-bottom: 12px; opacity: 0.75; font-size: 0.85em;">RP 컨설턴트 곽두철.</div>
            <div style="margin-bottom: 10px;">
                <label style="display:block; margin-bottom:4px; font-size:0.9em;">연결 프로필</label>
                <select id="gwak-ext-profile" class="text_pole" style="width:100%;">${profileOptions}</select>
            </div>
            <div style="display:flex; gap:8px; margin-bottom:10px;">
                <div style="flex:1;"><label style="font-size:0.9em;">최대 토큰</label><input type="number" id="gwak-ext-max-tokens" class="text_pole" style="width:100%;" value="${settings.maxTokens}"></div>
                <div style="flex:1;"><label style="font-size:0.9em;">참조 N개</label><input type="number" id="gwak-ext-recent-n" class="text_pole" style="width:100%;" value="${settings.recentChatN}"></div>
            </div>
            <button id="gwak-ext-save" class="menu_button" style="width: 100%; margin-bottom:10px;">💾 설정 저장</button>
            <button id="gwak-ext-open-btn" class="menu_button" style="width: 100%; margin-bottom:6px;">🐗 곽두철 패널 열기</button>
            <button id="gwak-ext-reset-panel" class="menu_button" style="width: 100%; opacity: 0.7; font-size: 0.85em;">↺ 패널/위치 리셋</button>
        </div>
    </div>`;
    ...
    document.getElementById('gwak-ext-save').addEventListener('click', () => {
        ...
        saveSettings(s); if (window.toastr) window.toastr.success('🐗 설정 저장됨');
    });
    ...
    document.getElementById('gwak-ext-reset-panel').addEventListener('click', () => {
        const s = loadSettings(); s.panelWidth = 380; s.panelHeight = 540; saveSettings(s);
        if (panel) panel.remove(); panel = null; if (window.toastr) window.toastr.success('🐗 리셋 완료');
    });
}

let wandRetryCount = 0;
export function addMenuEntry() { addWandMenuEntry(); addFloatingButton(); }
function addWandMenuEntry() {
    const menu = document.getElementById('extensionsMenu');
    if (menu) {
        if (document.getElementById('gwak-menu-entry')) return;
        const item = document.createElement('div');
        item.id = 'gwak-menu-entry'; item.classList.add('list-group-item');
        item.style.cursor = 'pointer'; item.innerHTML = `🐗 <span>곽두철</span>`;
        item.addEventListener('click', () => { togglePanel(); if (window.jQuery) window.jQuery('#extensionsMenu').fadeOut(200); });
        menu.appendChild(item); return;
    }
    if (wandRetryCount < 30) { wandRetryCount++; setTimeout(addWandMenuEntry, 500); }
}

function addFloatingButton() {
    if (document.getElementById('gwak-fab')) return;
    const fab = document.createElement('button');
    fab.id = 'gwak-fab'; fab.innerHTML = '🐗'; document.body.appendChild(fab);
    makeFabDraggable(fab);
}

function makeFabDraggable(fab) {
    let isDragging = false; let hasMoved = false; let startX, startY, startL, startT;
    function start(clientX, clientY) { const rect = fab.getBoundingClientRect(); startL = rect.left; startT = rect.top; startX = clientX; startY = clientY; isDragging = true; hasMoved = false; }
    function move(clientX, clientY) {
        if (!isDragging) return; const dx = clientX - startX; const dy = clientY - startY;
        if (Math.abs(dx) > 5 || Math.abs(dy) > 5) hasMoved = true;
        if (hasMoved) { fab.style.left = (startL + dx) + 'px'; fab.style.top = (startT + dy) + 'px'; fab.style.right = 'auto'; fab.style.bottom = 'auto'; }
    }
    fab.addEventListener('mousedown', (e) => { e.preventDefault(); start(e.clientX, e.clientY); });
    document.addEventListener('mousemove', (e) => move(e.clientX, e.clientY));
    document.addEventListener('mouseup', () => { if (isDragging && !hasMoved) togglePanel(); isDragging = false; });
    ...
    document.addEventListener('touchend', () => { if (isDragging && !hasMoved) togglePanel(); isDragging = false; });
}
